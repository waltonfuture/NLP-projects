...
class GLUETrainer(Trainer):
    ...
    def _save_best_checkpoint(self, best_step):
        model = PlmForSequenceClassification.from_pretrained(
...
def main():
    ...
    config = PlmConfig.from_pretrained(
        model_args.config_name
        if model_args.config_name else model_args.model_name_or_path,
    )
    config.num_labels = num_labels
    config.use_fofe = True
    config.every_layer = True
    config.alpha = 0.0
    config.fofe_mode = 'add'
    config.parallel = True
    config.fofe_dim = 128
    config.window_size = 5
    config.is_conv1d = True
    config.gumbel_tau = 0.1
    config.kl_beta = 0.0
    ...
    temp_model = BertForSequenceClassification.from_pretrained(model_args.model_name_or_path,
                                                               num_labels=num_labels, )

    for key in temp_model.state_dict().keys():
        temp_name = key.replace('bert', 'plm.bertEncoder', 1)
        if temp_name in model.state_dict().keys():
            model.state_dict()[temp_name].copy_(temp_model.state_dict()[key])

    '''
    if config.freeze == True:
        # 冻住原bert参数
        unfreeze_layers = ['classifier', 'fofe_layer', 'window_weights']
