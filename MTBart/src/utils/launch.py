...
def main():
    ...
    for local_rank in range(0, args.nproc_per_node):
        ...
        if not args.use_env:
            ...
            cmd.append("--global_rank={}".format(dist_rank))
        ...
        # # deal with 0MB process problem in slurm
        # total_devices = os.environ['CUDA_VISIBLE_DEVICES'].split(',')
        # # cmd = ' '.join(cmd)
        # cmd = f"CUDA_VISIBLE_DEVICES={total_devices[local_rank]} " + ' '.join(cmd)
            ...
            for process in alive_processes:
                if process.poll() is None:
                    ...
                    if process.returncode != 0:
                        ...
                        sigkill_handler(signal.SIGTERM,
                                        None)  # not coming back
                    else:
                        # exited cleanly
