...
class MTGenerationTrainer(Trainer):
    ...
    def __init__(self, decoders=['L2R'], *args, **kwargs):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        for decoderType in self.decoders:
            labels[decoderType] = inputs['decoder_input_ids'][decoderType]
        '''
        for decoderType in self.decoders:
            labels[decoderType] = inputs['decoder_input_ids'].pop(decoderType)

        for decoderType in ['L2R', 'R2L', 'BERT']:
            if decoderType in inputs:
                inputs['decoder_input_ids'].pop(decoderType)
        '''
